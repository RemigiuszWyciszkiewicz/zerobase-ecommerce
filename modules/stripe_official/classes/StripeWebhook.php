...
class StripeWebhook
{
    ...
    public function createStripeWebhook()
    {
        ...
        try {
            ...
            StripeProcessLogger::debugLog('createStripeWebhook ' . json_encode($data));
            $webhookEndpoint = $this->stripeClient->webhookEndpoints->create($data);
        } catch (Exception $e) {
    ...
    public function updateStripeWebhook($webhookId)
    {
        ...
        try {
            ...
            if (!empty($webhookUpdateData)) {
                StripeProcessLogger::debugLog("updateStripeWebhook with id: $webhookId data: " . json_encode($webhookUpdateData));
    ...
    public function deleteStripeWebhook($webhookId)
    {
        $deleted = false;
        try {
            StripeProcessLogger::debugLog("deleteStripeWebhook with id: $webhookId ");
    ...
    public function getWebhookList()
    {
        ...
        try {
            ...
            StripeProcessLogger::debugLog('getWebhookList response: ' . json_encode($webhookEndpointsList));
        } catch (Exception $e) {
    ...
    {
        StripeProcessLogger::debugLog('updateWebhookDataInDB ' . json_encode($webhook));
    ...
    public function registerWebhook()
    {
        ...
        StripeProcessLogger::debugLog('registerWebhook ' . json_encode(['shopGroupId' => $shopGroupId, 'shopId' => $shopId, 'webhookId' => $webhookId]));
    ...
    private function checkIfExistsWebhookEndpoints($webhookId)
    {
        ...
        StripeProcessLogger::debugLog("checkIfExistsWebhookEndpoints called with $webhookId");
        if ($webhookId) {
            $existsWebhookEndpoint = $this->retrieveWebhookEndpoints($webhookId);
        }

        StripeProcessLogger::debugLog("checkIfExistsWebhookEndpoints with $webhookId found the existing:" . json_encode($existsWebhookEndpoint));
        if (!$existsWebhookEndpoint) {
            $stripeAccountDetails = new StripeAccountDetails();
            $stripeAccountDetails->getByIdStripeAccount($accountId);
            if ($stripeAccountDetails->id_stripe_account) {
                ...
                StripeProcessLogger::debugLog("checkIfExistsWebhookEndpoints retrieved from stripe account: $webhookId with signature: $webhookSignature and existing: " . json_encode($existsWebhookEndpoint));

                if ($existsWebhookEndpoint) {
                    StripeProcessLogger::debugLog("checkIfExistsWebhookEndpoints updating configuration with id: $webhookId and signature: $webhookSignature");
