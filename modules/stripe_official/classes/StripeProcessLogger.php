...
{
    public static function debugLog($message, $cartId = '')
    {
        // TODO find a way to make this configurable so even clients can enable it
        $debug = false;
        if (!$debug) {
            return;
        }
        $debugMessage = self::getCallerString() . ' - ' . $message;
        \PrestaShopLogger::addLog(self::getFormattedMessageLogs($debugMessage, ''), 1, null, 'debugLog', $cartId, true);
    }
