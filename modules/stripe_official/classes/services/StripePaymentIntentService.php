...
class StripePaymentIntentService
{
    ...
    public function buildPaymentIntentParams($cartContextModel, $separateAuthAndCapture, $orderModel = null, $checkoutSession = false)
    {
        ...
        if (in_array(Tools::strtolower($cartContextModel->currencyIsoCode), ['ugx'])) {
            $amount = $cartContextModel->amount * 100;
        }
    ...
    {
        if (!$paymentIntentId || strncmp($paymentIntentId, 'pi_', strlen('pi_')) !== 0) {
            return null;
        }
