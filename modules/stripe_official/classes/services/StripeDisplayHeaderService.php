...
class StripeDisplayHeaderService
{
    ...
    public function setHeaders()
    {
        ...
        $productData = $this->context->smarty->tpl_vars['product']->value ?? null;
        $productId = $productData->id_product ?? $productData['id_product'] ?? null;
        ...
        $merchantCountry = new \Country((int) \Configuration::get('PS_COUNTRY_DEFAULT'));
        $stripeFullName = $prestashopVersion = '';
        if (version_compare(_PS_VERSION_, '1.7', '>=')) {
            ...
            $stripeFullName = $firstname . ' ' . $lastname;
        }
        ...
        \Media::addJsDef([
            ...
            'stripe_fullname' => $stripeFullName,
            ...
            'stripe_ps_version' => $prestashopVersion,
    ...
    private function shouldIncludeExpressCheckout(): bool
    {
        ...
        if (
            $expressCheckoutEnabled && strpos($expressCheckoutLocations, $controller) !== false
        ) {
            return true;
        }
