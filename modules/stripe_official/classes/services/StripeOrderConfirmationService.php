...
class StripeOrderConfirmationService
{
    ...
    public function orderConfirmationNew(?string $cartId = null)
    {
        ...
        try {
            ...
            $this->prestashopOrderService->removeProductsFromDuplicateCart(new Cart((int) $cartId));

            $stripeIdempotencyKey = StripeIdempotencyKey::getOrCreateIdempotencyKey((int) $cartId);
            ...
            $this->prestashopOrderService->updatePsStripePayment($intent, (int) $cartId);
    ...
    public function checkOrderAlreadyConfirmed(?string $cartId = null)
    {
        ...
        if (!$cartId) {
            ...
            throw new StripeOrderAlreadyConfirmedException($failUrl);
        }
        $payment_id = StripePayment::getIdByCartId((int) $cartId);
        if ($payment_id) {
            ...
            if (!empty($payment->id_stripe) && !empty($payment->id_payment_intent)) {
                ...
                throw new StripeOrderAlreadyConfirmedException($this->prestashopOrderService->getOrderConfirmationLink($order));
            }
        }
    }
}
