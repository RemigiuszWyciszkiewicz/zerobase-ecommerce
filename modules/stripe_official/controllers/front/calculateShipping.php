...
class stripe_officialCalculateShippingModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if (Context::getContext()->cart->id) {
            ...
            StripeProcessLogger::debugLog('calculateShipping: cart is fetched using id: ' . Context::getContext()->cart->id . " cartId: $cart->id", Context::getContext()->cart->id);
        }

        $idProductAttribute = null;
        if ($content['expressCheckoutType'] === 'product') {
            StripeProcessLogger::debugLog('calculateShipping: Express checkout type is product', Context::getContext()->cart->id);
            ...
            StripeProcessLogger::debugLog('calculateShipping: Cart is created and product added to cart ' . $cart->id, Context::getContext()->cart->id);
        }
        ...
        if ($currency && isset($currency->precision)) {
