...
class ProductComments extends Module
{
    ...
    private $templateFile = 'views/templates/front/productcomments_reviews.tpl';
    ...
    public function install($keep = true)
    {
        ...
        if (parent::install() == false || !$this->registerHook('displayCommentsExtra')  || !$this->registerHook('displayProductTab') || !$this->registerHook('displayProductTabContent') || !$this->registerHook('displayFooterProduct') || !$this->registerHook('displayProductAdditionalInfo') || !$this->registerHook('displayHeader') || !$this->registerHook('displayRightColumnProduct') || !$this->registerHook('displayProductListReviews') || !$this->registerHook('actionObjectProductCommentAddAfter') || !Configuration::updateValue('PRODUCT_COMMENTS_MINIMAL_TIME', 30) || !Configuration::updateValue('PRODUCT_COMMENTS_COMMENTS_PER_PAGE', 5) || !Configuration::updateValue('PRODUCT_COMMENTS_ALLOW_GUESTS', 0) || !Configuration::updateValue('PRODUCT_COMMENTS_MODERATE', 1)) {
            return false;
        }
    ...
    {
        return parent::getCacheId($this->name . '|' . (int)$id_product);
    }
...
//* v8 
    ...
    public function hookDisplayProductListReviews($params)
    {
        ...
        if (!$this->isCached($this->templateFile, $this->getCacheId($id_product))) {
            ...
            $nbComment = (int)ProductComment::getCommentNumber($id_product);

            $this->smarty->assign([
                'product'       => $params['product'],
                'averageTotal'  => round($average['grade'] ?? 0),
                'ratings'       => $ratings,
                'nbComments'    => $nbComment,
            ]);
        }

        return $this->display(__FILE__, $this->templateFile, $this->getCacheId($id_product));
    }
