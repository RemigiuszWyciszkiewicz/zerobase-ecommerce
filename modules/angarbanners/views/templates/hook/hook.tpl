...
{else}
	...
	<li class="angarbanners-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'} {if $hook == 'header' || $hook == 'footer'}col-xs-12{else} col-xs-4{/if}">
		...
		{/if}
			...
			{if $hItem.image}
				<img src="{$link->getMediaLink("`$module_dir`views/img/`$hItem.image|escape:'htmlall':'UTF-8'`")}" class="item-img {if $hook == 'left' || $hook == 'right'}img-responsive{/if}" title="{$hItem.title|escape:'htmlall':'UTF-8'}" alt="{$hItem.title|escape:'htmlall':'UTF-8'}" width="{if $hItem.image_w}{$hItem.image_w|intval}{else}100%{/if}" height="{if $hItem.image_h}{$hItem.image_h|intval}{else}100%{/if}" {if $hook == 'bottom'}loading="lazy"{/if} />
			{/if}
