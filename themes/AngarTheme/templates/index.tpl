...
<contact@prestashop.com>
  * @copyright 2007-2025 PrestaShop SA * @license
  ...
  AngarTheme *} {extends file='page.tpl'} {block name='page_content_container'}
  <section id="content" class="page-home">
    {if (!empty($css7) && $css7 == 'slider_position_column') OR empty($css7) OR
    $enableconfig == 1} {if $page.page_name == 'index'}
    <div id="slider_row" class="slider_column">
      <div id="top_column">{hook h="displayTopColumn"}</div>
      <div class="clearfix"></div>
      {hook h='angarHomeCat'}
    </div>
    {/if} {/if} {block name='page_content_top'}{/block} {hook
    h='displayAngarAboveTabs'} {block name='page_content'} {assign
    var='HOOK_HOME_TAB_CONTENT' value=Hook::exec('displayHomeTabContent')}
    {assign var='HOOK_HOME_TAB' value=Hook::exec('displayHomeTab')} {if
    isset($HOOK_HOME_TAB_CONTENT) && $HOOK_HOME_TAB_CONTENT|trim}
    <div class="tabs">
    ...
    {/if} {$HOOK_HOME nofilter} {/block}

    <div class="yt-preview-container">
      {hook h='displayYoutubeChannel'} {hook h='displayYoutubeShorts'}
    </div>
  </section>
  {/block}
</contact@prestashop.com>
