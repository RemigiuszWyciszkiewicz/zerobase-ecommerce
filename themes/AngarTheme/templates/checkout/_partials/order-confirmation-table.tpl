  ...
  <div class="order-confirmation-table">
    ...
    {block name='order_confirmation_table'}
      ...
      <table>
		...
		{if isset($subtotal) && $subtotal !== null && isset($subtotal.type) && $subtotal.type !== 'tax' && isset($subtotal.label) && $subtotal.label !== null}
		  <tr>
		...
		{/if}
        {/foreach}
		{if isset($subtotals.tax) && $subtotals.tax.label !== null}
		  <tr class="sub">
		...
		{if $smarty.const._PS_VERSION_ >= '1.7.7.0'}
			...
			{else}
				<tr><td></td><td></td></tr>
				<tr class="total-value font-weight-bold">
					<td><span class="text-uppercase">{$totals.total.label}&nbsp;{if isset($configuration.taxes_enabled) && $configuration.taxes_enabled}{$labels.tax_short}{/if}</span></td>
					<td>{$totals.total.value}</td>
				</tr>
			{/if}
		{else}
